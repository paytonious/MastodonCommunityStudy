...
response = requests.get("https://fosstodon.org/api/v1/trends/tags?limit=20")
...
for status in statusesList:
    ...
    response = requests.get("https://fosstodon.org/api/v1/timelines/tag/" + tagName + "?limit=40")
    postsList = json.loads(response.text)

    flattened_posts = json_normalize(postsList)

    tagPostString = "Fosstodon_" + tagName + "_post.csv"
